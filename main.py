...
def system_calculation(): # основная функция расчета модели 
    ...
    for i in range(1, 2*N): # перебераем кол маятников
        Y0.append(0) # задаем ноль

    def f(Y, t): # функция систамы уравнений

        # Уравнения x' = Vx
        FY = [Y[N]] # левый маятник
        for i in range(1, N):
            FY.append(Y[N+i]) # остальные маятники

        # Уравнения Vx' = sum(F) / m
        FY.append(F0 * cos(w * t + fi) + k * (Y[1] - Y[0]) / 1) # левый маятник
        for i in range(1, N - 1):
            FY.append(k * (Y[i - 1] - 2 * Y[i] + Y[i + 1]) / 1) # средние маятники
        FY.append(k * (Y[N - 2] - Y[N - 1]) / 1) # правый маятник
        return FY
